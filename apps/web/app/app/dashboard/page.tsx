...
import { getCourses, CourseItem } from '../../../lib/coursesData';
    ...
    <div className="space-y-8">
      <div className="space-y-2">
        ...
        <p className="text-xs text-slate-400 font-mono">
          {activeCourse ? `${activeCourse.cohort} · ${activeCourse.courseName}` : 'Cohort 1 · Foundation Certificate in Mediation'}
        </p>
      </div>
      ...
      {activeCourse && (
        <Card variant="bordered" className="p-8 bg-slate-900/90 border-amber-500/30 space-y-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <Badge variant="gold" className="font-mono">{activeCourse.badge}</Badge>
              <h2 className="text-2xl font-serif font-bold text-white mt-2">
                {activeCourse.courseName}
              </h2>
              <p className="text-xs text-slate-400 mt-1">{activeCourse.cohort} · {activeCourse.duration} · {activeCourse.jurisdiction}</p>
            </div>
            <Badge variant="success">IN PROGRESS ({activeCourse.progressPercent}%)</Badge>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs font-mono text-slate-300">
              ...
              <span className="text-amber-400 font-bold">{activeCourse.completedHours} / {activeCourse.totalHours} Hours Completed</span>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 text-xs font-mono text-slate-300">
            ...
            <div className="bg-slate-950 p-3.5 rounded border border-slate-800">
              ...
              <span className="text-white font-semibold">{activeCourse.mentorName}</span>
            </div>
            <div className="bg-slate-950 p-3.5 rounded border border-slate-800">
          ...
          <div className="pt-4 flex gap-4">
            <Link href="/app/enrollments">
              <Button variant="primary" className="text-xs uppercase tracking-wider py-2.5 px-6 font-bold shadow-md shadow-amber-900/30">
                Continue Learning ({activeCourse.modules.length} Modules) →
              </Button>
            </Link>
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card variant="default" className="p-6 space-y-2">
          <div className="text-2xl font-serif font-bold text-white">4</div>
          <div className="text-xs font-mono text-slate-400 uppercase">Available Academic Programs</div>
        </Card>
        <Card variant="default" className="p-6 space-y-2">
